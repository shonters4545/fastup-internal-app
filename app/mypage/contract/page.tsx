...
export default function ContractPage() {
  ...
  const renderMainContract = () => (
    <div className="bg-gray-50 dark:bg-gray-800/50 p-6 rounded-btn shadow-md">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">本契約</h2>
      {!contract ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          <div>
            ...
            <p className="mt-1 text-lg font-semibold text-gray-800 dark:text-gray-100">{contract.parent_email || '未設定'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-1 text-lg text-gray-800 dark:text-gray-100">{formatPaymentMethod(contract.payment_method)}</p>
          </div>
          <div className="col-span-1 md:col-span-2">
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">契約更新について</p>
            <p className={`mt-1 text-base ${contract.cancel_at_period_end ? 'text-warning-600 dark:text-warning-400' : 'text-gray-800 dark:text-gray-100'}`}>
      ...
      {items.length > 0 ? (
        <ul className="space-y-2 list-disc list-inside">
          {items.map((item, index) => (
            <li key={index} className="text-lg text-gray-800 dark:text-gray-100">{item}</li>
          ))}
        </ul>
      ) : (
  ...
  return (
    <div className="w-full max-w-4xl card p-8 animate-fade-in mt-8">
      <div className="flex justify-between items-center mb-8 border-b dark:border-gray-800 pb-4">
        <h1 className="text-2xl font-bold text-gray-700 dark:text-gray-100 tracking-wider">契約情報</h1>
