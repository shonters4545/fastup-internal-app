  ...
  return (
    <div className="w-full max-w-5xl card p-8 animate-fade-in mt-8">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 border-b dark:border-gray-800 pb-4 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-700 dark:text-gray-100 tracking-wider">学習アナリティクス</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">{studentName}さんの学習記録</p>
        </div>
        <div className="flex items-center gap-2 self-start sm:self-center">
          <button onClick={() => setTimeRange('7')} className={`px-4 py-2 text-sm font-medium rounded-btn ${timeRange === '7' ? 'bg-primary-600 text-white' : 'bg-gray-200 dark:bg-gray-800'}`}>過去7日間</button>
          <button onClick={() => setTimeRange('30')} className={`px-4 py-2 text-sm font-medium rounded-btn ${timeRange === '30' ? 'bg-primary-600 text-white' : 'bg-gray-200 dark:bg-gray-800'}`}>過去30日間</button>
          <Link href="/mypage" className="text-sm text-primary-600 dark:text-gray-400 hover:underline ml-4">&larr; 戻る</Link>
        </div>
      </div>
      ...
      {error ? (
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
            <div className="bg-primary-100 dark:bg-gray-900/40 p-6 rounded-btn"><h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">総学習時間</h3><p className="text-3xl font-bold text-primary-600 dark:text-gray-400 mt-2">{formatSecondsToHoursMinutes(analyticsData.totalTime)}</p></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-3 bg-gray-50 dark:bg-gray-800/50 p-6 rounded-btn relative">
              ...
              <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">日別学習時間</h3>
              <div className="flex h-48">
                ...
                {/* Chart bars */}
                <div className="flex-1 flex justify-around items-end gap-1 border-l border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="lg:col-span-2 bg-gray-50 dark:bg-gray-800/50 p-6 rounded-btn">
              <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">科目別学習割合</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Recent Activity */}
            <div>
              <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">最近完了したタスク</h3>
                ...
                <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                  {analyticsData.recentTasks.length > 0 ? analyticsData.recentTasks.map(task => (
                    <li key={task.id} className="py-3 flex items-center gap-4">
                      {task.bookImageUrl ? (
                        <img src={task.bookImageUrl} alt="" className="w-10 h-14 object-contain rounded-input flex-shrink-0 bg-gray-200" />
                      ) : (
                        <div className="w-10 h-14 bg-gray-200 dark:bg-gray-700 rounded-input flex-shrink-0"></div>
                      )}
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">最近のテスト結果</h3>
              </div>
              <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-btn">
                <ul className="divide-y divide-gray-200 dark:divide-gray-700">
          ...
          {/* Attendance Record */}
          <div>
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">出席記録</h3>
            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-btn">
              <ul className="divide-y divide-gray-200 dark:divide-gray-700">
