...
export default function AnalyticsPage() {
  ...
  return (
    <div className="w-full max-w-5xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 animate-fade-in mt-8">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 border-b dark:border-gray-700 pb-4 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white">学習アナリティクス</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">{studentName}さんの学習記録</p>
        </div>
        <div className="flex items-center gap-2 self-start sm:self-center">
          <button onClick={() => setTimeRange('7')} className={`px-4 py-2 text-sm font-medium rounded-md ${timeRange === '7' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>過去7日間</button>
          <button onClick={() => setTimeRange('30')} className={`px-4 py-2 text-sm font-medium rounded-md ${timeRange === '30' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>過去30日間</button>
          <Link href="/mypage" className="text-sm text-blue-600 dark:text-blue-400 hover:underline ml-4">&larr; 戻る</Link>
        </div>
      </div>

      {error ? (
        <div className="text-center p-8 bg-red-100 dark:bg-red-900/50 rounded-lg"><p className="text-red-600 dark:text-red-300">{error}</p></div>
      ) : analyticsData && (
        <div className="space-y-8">
          {/* Summary Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
            <div className="bg-indigo-100 dark:bg-indigo-900/40 p-6 rounded-lg"><h3 className="text-lg font-semibold text-indigo-800 dark:text-indigo-200">総学習時間</h3><p className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 mt-2">{formatSecondsToHoursMinutes(analyticsData.totalTime)}</p></div>
            <div className="bg-teal-100 dark:bg-teal-900/40 p-6 rounded-lg"><h3 className="text-lg font-semibold text-teal-800 dark:text-teal-200">完了タスク数</h3><p className="text-3xl font-bold text-teal-600 dark:text-teal-400 mt-2">{analyticsData.totalTasks}</p></div>
            <div className="bg-amber-100 dark:bg-amber-900/40 p-6 rounded-lg"><h3 className="text-lg font-semibold text-amber-800 dark:text-amber-200">平均学習時間/日</h3><p className="text-3xl font-bold text-amber-600 dark:text-amber-400 mt-2">{formatSecondsToHoursMinutes(analyticsData.avgTimePerDay)}</p></div>
            <div className="bg-rose-100 dark:bg-rose-900/40 p-6 rounded-lg"><h3 className="text-lg font-semibold text-rose-800 dark:text-rose-200">平均テスト点数</h3><p className="text-3xl font-bold text-rose-600 dark:text-rose-400 mt-2">{analyticsData.avgTestScore !== null ? `${analyticsData.avgTestScore}点` : '-'}</p></div>
          </div>

          {/* Charts */}
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-3 bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg relative">
              {tooltip && tooltip.visible && (
                <div
                  className="absolute bg-gray-900 text-white text-xs rounded py-1 px-2 pointer-events-none z-10 transform -translate-y-full -translate-x-1/2"
                  style={{ top: tooltip.y, left: tooltip.x }}
                >
                  {tooltip.content}
                </div>
              )}
              <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">日別学習時間</h3>
              <div className="flex h-48">
                {/* Y-axis labels */}
                <div className="relative w-12 h-full text-xs text-right pr-2 text-gray-400 dark:text-gray-500">
                ...
                {/* Chart bars */}
                <div className="flex-1 flex justify-around items-end gap-1 border-l border-b border-gray-200 dark:border-gray-600">
                    ...
                    return (
                      ...
                      >
                        <div className="w-full flex-grow flex items-end justify-center">
                          <div className="chart-bar-element w-3/4 bg-blue-500 rounded-t-sm hover:bg-blue-400 transition-colors" style={{ height: barHeight }}></div>
                        </div>
                        <span className={`text-[10px] text-gray-500 dark:text-gray-400 mt-1 ${showLabel ? 'visible' : 'invisible'}`}>
                          {data.day}
                        </span>
                      </div>
                    );
            ...
            <div className="lg:col-span-2 bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg">
              <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">科目別学習割合</h3>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                ...
                <ul className="space-y-2 text-sm">
                  {analyticsData.subjectData.length > 0 ? analyticsData.subjectData.map((item, index) => (
                    <li key={index} className="flex items-center"><div className="w-3 h-3 rounded-sm mr-2" style={{ backgroundColor: subjectColors[index % subjectColors.length] }}></div><span className="font-medium text-gray-700 dark:text-gray-300">{item.subject}: {formatSecondsToHoursMinutes(item.time)}</span></li>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Recent Activity */}
            <div>
              <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">最近完了したタスク</h3>
                ...
                <ul className="divide-y divide-gray-200 dark:divide-gray-600">
                  {analyticsData.recentTasks.length > 0 ? analyticsData.recentTasks.map(task => (
                    <li key={task.id} className="py-3 flex items-center gap-4">
                      {task.bookImageUrl ? (
                        <img src={task.bookImageUrl} alt="" className="w-10 h-14 object-contain rounded-md flex-shrink-0 bg-gray-200" />
                      ) : (
                        <div className="w-10 h-14 bg-gray-200 dark:bg-gray-600 rounded-md flex-shrink-0"></div>
                      )}
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 flex-shrink-0">{task.completedAt.toLocaleDateString('ja-JP')}</p>
                    </li>
                  )) : <p className="text-center text-gray-500 dark:text-gray-400 py-4">この期間に完了したタスクはありません。</p>}
                </ul>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800 dark:text-white">最近のテスト結果</h3>
              </div>
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                <ul className="divide-y divide-gray-200 dark:divide-gray-600">
                  {analyticsData.recentTestResults.length > 0 ? analyticsData.recentTestResults.map(test => (
                    <li key={test.id} className="py-3 flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">{test.completedAt.toLocaleDateString('ja-JP')}</p>
                      </div>
                      <div className="flex items-center">
                        <span className={`text-lg font-bold ${test.score >= 80 ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>
                          {test.score}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 ml-1">点</span>
                      </div>
                    </li>
                  )) : <p className="text-center text-gray-500 dark:text-gray-400 py-4">この期間のテスト結果はありません。</p>}
                </ul>
              </div>
            </div>
          </div>
          ...
          {/* Attendance Record */}
          <div>
            <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">出席記録</h3>
            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
              <ul className="divide-y divide-gray-200 dark:divide-gray-600">
                {analyticsData.attendanceRecords.length > 0 ? analyticsData.attendanceRecords.map(record => (
                  <li key={record.id} className="py-3 flex justify-between items-center">
                    <div>
                      <p className="font-medium text-gray-800 dark:text-gray-200">{record.className}</p>
                    </div>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{record.attendedAt.toLocaleDateString('ja-JP')}</p>
                  </li>
                )) : <p className="text-center text-gray-500 dark:text-gray-400 py-4">この期間の出席記録はありません。</p>}
              </ul>
            </div>
          </div>
        </div>
