    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {specials.map(special => {
          ...
          const cardContent = (
            <div className={`bg-white dark:bg-gray-900 rounded-card shadow-card overflow-hidden h-full flex flex-col transition-all duration-300 ${statusInfo ? 'opacity-70' : 'transform group-hover:-translate-y-1 group-hover:shadow-card-hover'}`}>
              ...
              <div className="p-6 flex flex-col flex-grow">
                <h2 className={`text-xl font-bold text-gray-800 dark:text-gray-100 line-clamp-2 ${!statusInfo && 'group-hover:text-accent-600 dark:group-hover:text-accent-400 transition-colors duration-300'}`}>
                  {special.title}
                </h2>
  ...
  return (
    <div className="w-full max-w-6xl animate-fade-in mt-8 space-y-8 mx-auto px-4">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-gray-700 dark:text-gray-100 tracking-wider">特別講座一覧</h1>
