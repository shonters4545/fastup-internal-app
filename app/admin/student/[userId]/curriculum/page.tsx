...
export default function AdminStudentCurriculumPage() {
  ...
  const calculateBookAverageScore = useCallback((book: Book) => {
    ...
    for (let lap = 1; lap <= maxLaps; lap++) {
      bookTasks.forEach(task => {
        const key = `${task.id}_${lap}`;
        const p = userProgress.get(key);
        if (p && p.score !== null) { totalScore += p.score; scoreCount++; }
      });
    }
  ...
  const saveProgress = async (taskId: string, bookId: string, lap: number, score: number | null, isSkipped: boolean) => {
    if (!userId) return;
    try {
      const insertData = { user_id: userId, task_id: taskId, book_id: bookId, lap, status: 'completed', score: isSkipped ? null : score };
      const { data, error } = await (supabase.from('progress') as any)
        .upsert(insertData, { onConflict: 'user_id,task_id,lap' })
        .select()
        .single();
      if (error) throw error;
      setUserProgress(prev => { const n = new Map(prev); n.set(`${taskId}_${lap}`, data); return n; });
    } catch (error) {
  ...
  const deleteProgress = async (taskId: string, lap: number) => {
    ...
    try {
      const { error } = await (supabase.from('progress') as any)
        ...
        .eq('lap', lap);
      if (error) throw error;
      setUserProgress(prev => { const n = new Map(prev); n.delete(`${taskId}_${lap}`); return n; });
    } catch (error) {
  ...
  const handleCheckboxClick = async (task: Task, book: Book, lap: number, isChecked: boolean) => {
    if (isChecked) {
      ...
      // Admin always saves directly without test score modal
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 pb-20">
      ...
      <nav className="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-4 gap-2">
        ...
        <span>/</span>
        <span className="text-gray-800 dark:text-gray-100 font-medium">カリキュラム</span>
      </nav>

      <h1 className="text-2xl font-bold mb-6 dark:text-gray-100">{studentName} のカリキュラム進捗</h1>
              ...
              {divisionBooks.length === 0 ? (
                ...
                <div className="space-y-4">
                  {divisionBooks.map(book => {
                    ...
                    return (
                      <div key={book.id} className="bg-white dark:bg-gray-900 rounded-btn shadow-card overflow-hidden border border-gray-200 dark:border-gray-800">
                        {/* Accordion Header */}
                          ...
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-3">
                              {!book.is_custom ? (
                                ...
                                <img src={book.image_url} alt={book.name} className="w-12 h-16 object-cover rounded shadow-sm bg-gray-50" referrerPolicy="no-referrer" />
                              ) : (
