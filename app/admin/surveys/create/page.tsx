...
export default function AdminCreateSurveyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await (supabase.from('survey_models') as any).insert({
        title,
  ...
  return (
    <div className="w-full max-w-4xl card p-8 animate-fade-in mt-8">
      <div className="flex justify-between items-center mb-6 border-b dark:border-gray-800 pb-4">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">新規アンケート作成</h1>
        <Link href="/admin/surveys" className="text-sm text-primary-600 dark:text-gray-400 hover:underline">&larr; 一覧に戻る</Link>
      </div>
        ...
        <div>
          ...
          <div className="mt-2 space-y-3 p-3 border border-gray-200 dark:border-gray-700 rounded-input bg-gray-50 dark:bg-gray-950/50">
            {formFields.map((field) => (
              <div key={field.id} className="grid grid-cols-12 gap-2 items-start p-2 bg-white dark:bg-gray-900 rounded-input shadow-sm">
                <div className="col-span-12 sm:col-span-4">
                  <input type="text" placeholder="質問文" value={field.label} onChange={(e) => handleFieldChange(field.id, 'label', e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-input text-sm text-gray-800 dark:text-gray-100 bg-white dark:bg-gray-800" required />
                </div>
                <div className="col-span-12 sm:col-span-2">
                  <select value={field.type} onChange={(e) => handleFieldChange(field.id, 'type', e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-input text-sm text-gray-800 dark:text-gray-100 bg-white dark:bg-gray-800">
                    <option value="text">テキスト</option>
                    <option value="select">セレクト</option>
                  </select>
                </div>
                <div className="col-span-12 sm:col-span-2 flex items-center h-full pl-2">
                  <input type="checkbox" checked={field.required} onChange={(e) => handleFieldRequiredChange(field.id, e.target.checked)} id={`required-${field.id}`} className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
                    ...
                    <div className="space-y-2">
                      {field.options.map((option, optionIndex) => (
                        <div key={optionIndex} className="flex items-center gap-2">
                          <input type="text" placeholder={`選択肢 ${optionIndex + 1}`} value={option} onChange={(e) => handleOptionChange(field.id, optionIndex, e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-700 rounded-input text-sm text-gray-800 dark:text-gray-100 bg-white dark:bg-gray-800" />
                      ...
                      <button type="button" onClick={() => handleAddOption(field.id)} className="text-xs text-primary-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-300 font-semibold">+ 選択肢を追加</button>
                    </div>
                  )}
                </div>
            ...
            <button type="button" onClick={handleAddField} className="mt-2 text-sm text-primary-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-300 font-semibold">+ 項目を追加する</button>
          </div>
        </div>
